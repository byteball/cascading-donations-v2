  ...
  return <Combobox as="div" className={cn("w-full", className)} value={value} onChange={(obj: ISearchRepoItem) => {
    ...
    <div className="relative">
      ...
      <Combobox.Input
        ...
        className={cn({"pl-8": showSearchIcon}, "w-full h-[40px] rounded-md border-0 bg-white py-2 pr-12 text-gray-900 shadow-sm ring-1  ring-gray-300 focus:ring-2 sm:leading-6", size === "large" ? "py-3 text-md" : "text-sm ", { "ring-primary ring-2": type === "primary" }, {"ring-red-500": error })}
        onChange={debounce((event) => setQuery(event.target.value), 800)}
      />
