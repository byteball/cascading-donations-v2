...
import { INotification, dismissNotification, selectNotifications } from "@/store/slices/notificationsSlice";
...
export const Notifications = () => {
  ...
  return (
    <div className="fixed top-0 z-50 right-0 max-w-[100%] w-[350px]">
      {/* Global notification live region, render this permanently at the end of the document */}
      {notifications.length > 0 && notifications.map((obj: INotification) => <Notification key={obj.id} {...obj} id={obj.id || 404} />)}
    </div>
  );
};
...
const Notification: FC<NotificationProps> = ({ id, title = "", description, type = "success" }) => {
  ...
  return (
    <div aria-live="assertive" className="relative w-[100%] inset-0 flex items-end px-4 py-3 pointer-events-none sm:items-start sm:p-3">
      ...
      <Transition
        show={!!notifications.find((n: INotification) => n.id === id)}
        as={Fragment}
        ...
        leaveTo="opacity-0"
      >
        <div className="w-full max-w-sm overflow-hidden rounded-lg shadow-xl pointer-events-auto bg-white ring-1 ring-black ring-opacity-5">
          <div className="p-4">
